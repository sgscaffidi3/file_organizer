...
# Import the existing classes from base_assets.py
...
def run_demo():
    ...
    pbar_outer = tqdm(total=len(asset_files), desc="OVERALL PROGRESS", position=0)
    ...
    with db:
        for file_path in asset_files:
            ...
            try:
                ...
                if content_hash == "PERMISSION_DENIED": continue

                raw_meta = {"OS_File_Size": file_path.stat().st_size}
                ...
                if ext in ['.jpg', '.jpeg', '.png', '.bmp']:
                    ...
                    media_group = "AUDIO"
                    raw_meta.update(get_video_metadata(file_path)) # Using same extractor for tags
                    asset = AudioAsset(file_path, raw_meta) # Using AudioAsset from base_assets
                else:
                    media_group = "GENERIC"
                ...
                new_meta_json = asset.get_full_json() # backpack remains exhaustive
                
                if old_meta is None:
                    stats['new'] += 1
                    # Extract values based on what AudioAsset/ImageAsset/VideoAsset provides
                    w = getattr(asset, 'width', 0)
                    h = getattr(asset, 'height', 0)
                    d = getattr(asset, 'duration', 0)
                    b = getattr(asset, 'bitrate', "Unknown")
                    c = getattr(asset, 'video_codec', "N/A")
                    ...
                    """, (content_hash, asset.size_bytes, media_group, w, h, d, b, c, new_meta_json))
                else:
